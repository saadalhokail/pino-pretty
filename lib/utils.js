 ...
 */
function prettifyLevel({ log, colorizer = defaultColorizer, levelKey = LEVEL_KEY, CCOLORED, CLEVELS, CLEVEL_NAMES }) {
  if (levelKey in log === false) return undefined
  return colorizer(log[levelKey], CCOLORED, CLEVELS, CLEVEL_NAMES)
}
 ...
 */
function prettifyMessage({ log, messageFormat, messageKey = MESSAGE_KEY, colorizer = defaultColorizer, levelLabel = LEVEL_LABEL, cLEVELS = LEVELS }) {
  if (messageFormat && typeof messageFormat === 'string') {
    const message = String(messageFormat).replace(/{([^{}]+)}/g, function (match, p1) {
      // return log level as string instead of int
      if (p1 === levelLabel && log[LEVEL_KEY]) {
        return cLEVELS[log[LEVEL_KEY]]
      }
